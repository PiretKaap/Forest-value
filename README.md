...
# Research Plan 
The main question is how to determine a forests lands true value. For this, we will: 

Understand the calculations of the program in use. XML (for structure/configuration) and EXCEL (for data tables and formulas). 

Isolate and record the fixed values, constants, or unchanging initial parameters that remain constant across all calculations or scenarios.  EXCEL 

Define the changeable (variable) initial data. EXCEL (for data organization) + Py (Python for identification and dynamic handling). 

Create a base code that utilizes the cadastral data generated by Linda AI. API (data origin from) + JSON (data format) + PY (Python programming language) + JUPITER NOTEBOOK (interactive environment for development and documentation). 
